...
// --- 共通コンポーネント: 座席グリッド ---
const DeskGrid = ({ 
  isResult = false, rows, cols, desks, isShuffling, stoppingIndex, allNames, finalResults, onUpdateDesk, onSetRows, onSetCols, children 
}: { 
              ...
              {[...Array(cols)].map((_, c) => {
                ...
                return (
                  <div key={desk.id} style={{ 
                    ...
                    backgroundColor: desk.isExcluded ? '#1a1a1a' : '#fff', overflow: 'hidden'
                  }}>
                    {!desk.isExcluded && (
                      isResult ? (
                        <SlotReel 
        ...
        <section style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <DeskGrid isResult={true} rows={rows} cols={cols} desks={desks} isShuffling={isShuffling} stoppingIndex={stoppingIndex} allNames={students} finalResults={finalResultsRef.current} onUpdateDesk={updateDesk} onSetRows={setRows} onSetCols={setCols}>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '20px' }}>
              <button onClick={startShuffle} disabled={isShuffling} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '15px 40px', borderRadius: '4px', border: '3px solid #000', backgroundColor: isShuffling ? '#ccc' : '#2563eb', color: '#fff', fontWeight: 'bold', fontSize: '20px', cursor: isShuffling ? 'not-allowed' : 'pointer' }}><Play size={24}/> スロット開始</button>
              <button onClick={stopNext} disabled={!isShuffling} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 25px', borderRadius: '4px', border: '3px solid #000', backgroundColor: !isShuffling ? '#eee' : '#f97316', color: !isShuffling ? '#999' : '#fff', fontWeight: 'bold', fontSize: '16px', cursor: !isShuffling ? 'not-allowed' : 'pointer' }}><SkipForward size={20}/> 次を止める</button>
              <button onClick={forceStop} disabled={!isShuffling} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 25px', borderRadius: '4px', border: '3px solid #000', backgroundColor: !isShuffling ? '#eee' : '#ef4444', color: !isShuffling ? '#999' : '#fff', fontWeight: 'bold', fontSize: '16px', cursor: !isShuffling ? 'not-allowed' : 'pointer' }}><Square size={20}/> 一括停止</button>
            </div>
          </DeskGrid>
        </section>
      </div>
